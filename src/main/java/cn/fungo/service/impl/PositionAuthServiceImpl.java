...
public class PositionAuthServiceImpl implements PositionAuthService {
	 ...
	 */
	@Override
	public Integer insertPositionAuth(List<AuthVO> list, String positionId) {
		...
		Integer cnt = mapper.insertPositionAuth(positionAuthList);
		return cnt;
	}
	 ...
	 */
	@Override
	public Integer updatePositionAuth(List<AuthVO> list, String positionId) {
		mapper.deleteAuthByPositionId(positionId);
		List<W2PositionAuth> positionAuthList = new ArrayList<>();
		for(AuthVO w : list) {
			W2PositionAuth obj = new W2PositionAuth();
			obj.setAuthId(w.getId());
